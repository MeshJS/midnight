...
export function initializeWallet() {
  const mnemonicToWords: (mnemonic: string) => string[] = (mnemonic: string) => mnemonic.split(' ');

  const generateMnemonicWords: (strength?: number) => string[] = (strength = 256) =>
    mnemonicToWords(bip39.generateMnemonic(english, strength));

  const validateMnemonic = (mnemonic: string): boolean => bip39.validateMnemonic(mnemonic, english);
  ...
  if (generatedWallet.type == 'seedOk') {
    ...
    if (zswapKey.type === 'keyDerived') {
      ...
      return { success: true, key: zswapKey.key };
    } else {
      console.error('Error deriving key');
      return { success: false, error: 'Error deriving key' };
    }
  } else {
    console.error('Error generating HDWallet');
    return { success: false, error: 'Error generating HDWallet' };
  }
}

// Uncomment this line if you want to run the wallet initialization when this module is imported
// initializeWallet();
