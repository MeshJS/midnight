...
import type { PrivateStates as CoinPrivateState } from '@meshsdk/token-api';
...
export class TestWallet {
  ...
  setup = async (testConfiguration: TestConfiguration) => {
    ...
    console.log(`CoinPK: ${state.coinPublicKey}`);
    return this.wallet;
  };
...
export class TestProviders {
  ...
  configureProviders = async (wallet: Wallet & Resource, config: Config) => {
    ...
    return {
      ...
      zkConfigProvider: new NodeZkConfigProvider<'start_bid' | 'close_bid' | 'approve_certificates' | 'register' | 'make_bid'>(
        config.auctionZkConfigPath,
      ),
  ...
  configureTokenProviders = async (wallet: Wallet & Resource, config: Config) => {
    const walletAndMidnightProvider = await this.createWalletAndMidnightProvider(wallet);
    const inMemory = inMemoryPrivateStateProvider<CoinPrivateState>();
    return {
      privateStateProvider: inMemory,
      publicDataProvider: indexerPublicDataProvider(config.indexer, config.indexerWS),
